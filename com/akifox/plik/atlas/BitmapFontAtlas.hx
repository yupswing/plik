...
class BitmapFontAtlas extends TextureAtlas
{
	...
	public static function getFont(fontName:String, ?format:BitmapFontFormat, ?extraParams:Dynamic):BitmapFontAtlas
	{
		...
		//if (fontName == defaultFont + ".png") format = XNA;
	...
	public static function loadXMLFont(file:String):BitmapFontAtlas
	{
		...
		{
			rect.x = Std.parseInt(char.att.x);
			rect.y = Std.parseInt(char.att.y);
			rect.width = Std.parseInt(char.att.width);
			rect.height = Std.parseInt(char.att.height);
			...
			var region = atlas.defineRegion(glyph, rect);
	...
	public static function loadXNAFont(asset:String, ?options:Dynamic):BitmapFontAtlas
	{
		...
		bmd.threshold(bmd, bmd.rect, zero, "==", bgColor32, 0x00000000, 0xFFFFFFFF, true);
		
		if (options.glyphBGColor != null)
			bmd.threshold(bmd, bmd.rect, zero, "==", options.glyphBGColor, 0x00000000, 0xFFFFFFFF, true);
